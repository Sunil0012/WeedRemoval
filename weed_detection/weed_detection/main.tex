...
\begin{tikzpicture}[
  ...
  \node[flow, right=of state] (momentum) {Compute Momentum\\and Reward};
  \node[flow, right=of momentum] (target) {Target ELO\\and Difficulty};

  % Middle decision
  \node[decision, below=1.5cm of momentum] (miss) {Unresolved Miss?};

  % Left & right branches
  \node[flow, left=2cm of miss] (remed) {Neighbors around\\Missed Question};
  \node[flow, right=2cm of miss] (curr) {Neighbors around\\Current Question};

  % Bottom flow
  \node[flow, below=1.5cm of miss] (pool) {Build Candidate Pool};
  \node[insight, below=1.2cm of pool] (score) {Heuristic Score\\+ Graph Boost};
  \node[insight, below=1.2cm of score] (out) {Next Question with\\Metadata \& Reasons};
  ...
  \draw[line] (miss) -- node[above]{\scriptsize yes} (remed);
  \draw[line] (miss) -- node[above]{\scriptsize no} (curr);
...
\begin{tikzpicture}[
  ...
  \node[flow, right=of wrong] (targetq) {Mark Unresolved\\Target Question};
  \node[flow, right=of targetq] (hops) {Serve Remediation\\Neighbors};

  % Tracking
  \node[flow, below=1.5cm of targetq] (track) {Track Accuracy\\and Steps};

  % Decision
  \node[decision, below=1.5cm of hops] (retry) {Enough Steps\\and Accuracy?};

  % Outcomes
  \node[insight, left=2cm of retry] (more) {Continue\\Remediation};
  \node[insight, right=2cm of retry] (serve) {Retry Original\\Question};
  ...
  \draw[line] (retry) -- node[above]{\scriptsize no} (more);
  \draw[line] (retry) -- node[above]{\scriptsize yes} (serve);
...
\begin{figure}[h]
    ...
    \caption{Adaptive Learning Graph showing question relationships and student progression}
\end{figure}
